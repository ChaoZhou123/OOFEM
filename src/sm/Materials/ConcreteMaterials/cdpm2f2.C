...
void
CDPM2F2Status::printOutputAt(FILE *file, TimeStep *tStep) const
{
    ...
    // print status flag
    switch ( state_flag ) {
    case CDPM2F2Status::CDPM2F2_Elastic:
        fprintf(file, "Elastic, ");
        break;
    case CDPM2F2Status::CDPM2F2_Unloading:
        fprintf(file, "Unloading, ");
        break;
    case CDPM2F2Status::CDPM2F2_Plastic:
        fprintf(file, "Plastic, ");
        break;
    case CDPM2F2Status::CDPM2F2_Damage:
        fprintf(file, "Damage, ");
        break;
    case CDPM2F2Status::CDPM2F2_PlasticDamage:
...
void
CDPM2F2Status::saveContext(DataStream &stream, ContextMode mode)
{
...
void
CDPM2F2Status::restoreContext(DataStream &stream, ContextMode mode)
{
...
#ifdef keep_track_of_dissipated_energy
void
CDPM2F2Status::computeWork(GaussPoint *gp, double gf)
{
...
bool
CDPM2F2::hasMaterialModeCapability(MaterialMode mode) const
//
...
CDPM2F2::initializeFrom(InputRecord &ir)
{
    ...
    IR_GIVE_OPTIONAL_FIELD(ir, this->isotropicFlag, _IFT_CDPM2F2_isoflag);
    ...
    IR_GIVE_FIELD(ir, this->fc, _IFT_CDPM2F2_fc);
    IR_GIVE_FIELD(ir, this->ft, _IFT_CDPM2F2_ft);

    this->e0 = this->ft / this->eM;

    // default parameters
    this->ecc = 0.525;
    IR_GIVE_OPTIONAL_FIELD(ir, this->ecc, _IFT_CDPM2F2_ecc);
    this->yieldHardInitial = 0.3;
    IR_GIVE_OPTIONAL_FIELD(ir, this->yieldHardInitial, _IFT_CDPM2F2_kinit);

    //Inclination at transition point
    this->yieldHardPrimePeak = 0.5;
    IR_GIVE_OPTIONAL_FIELD(ir, this->yieldHardPrimePeak, _IFT_CDPM2F2_hp);
    ...
    this->AHard = 8.e-2;
    IR_GIVE_OPTIONAL_FIELD(ir, this->AHard, _IFT_CDPM2F2_ahard);
    this->BHard = 3.e-3;
    IR_GIVE_OPTIONAL_FIELD(ir, this->BHard, _IFT_CDPM2F2_bhard);
    this->CHard = 2.;
    IR_GIVE_OPTIONAL_FIELD(ir, this->CHard, _IFT_CDPM2F2_chard);
    this->DHard = 1.e-6;
    IR_GIVE_OPTIONAL_FIELD(ir, this->DHard, _IFT_CDPM2F2_dhard);
    this->dilationConst = 0.85;
    IR_GIVE_OPTIONAL_FIELD(ir, this->dilationConst, _IFT_CDPM2F2_dilation);

    this->softeningType = 1; //0-Linear softening; 1-Bilinear softening; 2-Exponential
    IR_GIVE_OPTIONAL_FIELD(ir, this->softeningType, _IFT_CDPM2F2_softeningType);

    if ( softeningType > 2 ) {
        throw ValueInputException(ir, _IFT_CDPM2F2_softeningType, "softening type not implemented");
    }

    IR_GIVE_FIELD(ir, this->wf, _IFT_CDPM2F2_wf);

    if ( this->softeningType == 1 ) {
        this->ftOne = 0.3 * this->ft;
        IR_GIVE_OPTIONAL_FIELD(ir, this->ftOne, _IFT_CDPM2F2_ftOne);
        this->wfOne = 0.15 * this->wf;
        IR_GIVE_OPTIONAL_FIELD(ir, this->wfOne, _IFT_CDPM2F2_wfOne);
    }

    this->efCompression = 100.e-6;
    IR_GIVE_OPTIONAL_FIELD(ir, this->efCompression, _IFT_CDPM2F2_efc);

    this->ASoft = 15;
    IR_GIVE_OPTIONAL_FIELD(ir, this->ASoft, _IFT_CDPM2F2_asoft);

    this->helem = 0.;
    IR_GIVE_OPTIONAL_FIELD(ir, this->helem, _IFT_CDPM2F2_helem);
    ...
    this->yieldTol = 1.e-6;
    IR_GIVE_OPTIONAL_FIELD(ir, this->yieldTol, _IFT_CDPM2F2_yieldtol);

    this->yieldTolDamage = this->yieldTol * 10.;

    this->newtonIter = 100;
    IR_GIVE_OPTIONAL_FIELD(ir, this->newtonIter, _IFT_CDPM2F2_newtoniter);

    this->strengthRateType = 0;
    IR_GIVE_OPTIONAL_FIELD(ir, this->strengthRateType, _IFT_CDPM2F2_strengthratetype);
    ...
    if ( this->strengthRateType > 0 ) {
        IR_GIVE_OPTIONAL_FIELD(ir, this->deltaTime, _IFT_CDPM2F2_deltatime);
        IR_GIVE_OPTIONAL_FIELD(ir, this->energyRateType, _IFT_CDPM2F2_energyratetype);
    }
...
void
CDPM2F2::saveContext(DataStream &stream, ContextMode mode)
{
...
void
CDPM2F2::restoreContext(DataStream &stream, ContextMode mode)
{
...
#if 0
void
CDPM2F2::giveRealStressVector_1d(FloatArray &answer,
                                      GaussPoint *gp,
                                      const FloatArray &fullStrainVector,
                                      TimeStep *tStep)
{
    auto status = static_cast< CDPM2F2Status * >( this->giveStatus(gp) );
...
FloatArrayF< 6 >
CDPM2F2::giveRealStressVector_3d(const FloatArrayF< 6 > &fullStrainVector, GaussPoint *gp, TimeStep *tStep) const
{
    auto status = static_cast< CDPM2F2Status * >( this->giveStatus(gp) );
...
FloatArrayF< 2 >
CDPM2F2::computeDamage(const FloatArrayF< 6 > &strain,
...
int
CDPM2F2::checkForUnAndReloading(double &tempEquivStrain,
...
double
CDPM2F2::computeRateFactor(double alpha,
                                double deltaTime,
                                GaussPoint *gp,
...
double
CDPM2F2::computeDeltaPlasticStrainNormTension(double tempKappaD, double kappaD, GaussPoint *gp) const
{
    auto status = static_cast< CDPM2F2Status * >( this->giveStatus(gp) );
...
double
CDPM2F2::computeDeltaPlasticStrainNormCompression(double tempAlpha, double tempKappaD, double kappaD, GaussPoint *gp, const double rho) const
{
    auto status = static_cast< CDPM2F2Status * >( this->giveStatus(gp) );
...
double
CDPM2F2::computeEquivalentStrain(double sig,
                                      double rho,
                                      double theta) const
{
...
double
CDPM2F2::computeDamageParamCompression(double equivStrain, double kappaOne, double kappaTwo, double omegaOld, double rateFactor) const
{
    if ( isotropicFlag == 1 ) {
        return 0.;
    }
...
void
CDPM2F2::initDamaged(double kappaD,
...
double
CDPM2F2::computeDuctilityMeasureDamage(GaussPoint *gp, const double sig, const double rho) const
{
...
FloatArrayF< 6 >
CDPM2F2::performPlasticityReturn(GaussPoint *gp, const FloatMatrixF< 6, 6 > &D, const FloatArrayF< 6 > &strain) const
{
    auto status = static_cast< CDPM2F2Status * >( this->giveStatus(gp) );
    ...
    status->letTempReturnResultBe(CDPM2F2Status::RR_NotConverged);
    while ( status->giveTempReturnResult() == CDPM2F2Status::RR_NotConverged || subIncrementFlag == 1 ) {
        ...
        if ( yieldValue > 0. ) {
            ...
            if ( status->giveTempReturnType() == CDPM2F2Status::RT_Tension || status->giveTempReturnType() == CDPM2F2Status::RT_Compression ) {
                tempKappaP = performVertexReturn(effectiveStress, apexStress, tempKappaP, gp);
                status->letTempKappaPBe(tempKappaP);
            }
            if ( status->giveTempReturnType() == CDPM2F2Status::RT_Regular ) {
                tempKappaP = performRegularReturn(effectiveStress, tempKappaP, gp, theta);
                status->letTempKappaPBe(tempKappaP);
            }
        } else {
            status->letTempReturnResultBe(CDPM2F2Status::RR_Converged);
        ...
        if ( status->giveTempReturnResult() == CDPM2F2Status::RR_NotConverged ) {
            subincrementcounter++;
        ...
        } else if ( status->giveTempReturnResult() == CDPM2F2Status::RR_Converged && subIncrementFlag == 0 ) {
            auto C = inv(D); // compliance
            elasticStrain = dot(C, effectiveStress);
            tempPlasticStrain = strain - elasticStrain;
            status->letTempPlasticStrainBe(tempPlasticStrain);
        } else if ( status->giveTempReturnResult() == CDPM2F2Status::RR_Converged && subIncrementFlag == 1 ) {
...
void
CDPM2F2::checkForVertexCase(double &answer,
                                 double sig,
                                 double tempKappa,
                                 bool mat1d,
                                 GaussPoint *gp) const
{
    ...
    if ( mat1d ) {
        status->letTempReturnTypeBe(CDPM2F2Status::RT_Regular);
    }

    answer = 0.;
    if ( sig > 0. ) {
        status->letTempReturnTypeBe(CDPM2F2Status::RT_Tension);
    } else if ( sig < 0. &&  tempKappa < 1. ) {
        status->letTempReturnTypeBe(CDPM2F2Status::RT_Compression);
    } else {
        status->letTempReturnTypeBe(CDPM2F2Status::RT_Regular);
    }
}
...
double
CDPM2F2::performVertexReturn(FloatArrayF< 6 > &effectiveStress,
                                  ...
                                  GaussPoint *gp) const
{
    ...
    for ( int j = 0; j < 250; j++ ) {
        ...
        if ( fabs(yieldValueMid) < yieldTol && yieldValueMid <= 0. ) {
            ...
            if ( ( ( ( ratioPotential >= ratioTrial ) && status->giveTempReturnType() == CDPM2F2Status::RT_Tension ) ) ||
                 ( ( ratioPotential <= ratioTrial ) && status->giveTempReturnType() == CDPM2F2Status::RT_Compression ) ) {
                for ( int i = 0; i < 3; i++ ) {
...
double
CDPM2F2::computeTempKappa(double kappaInitial,
                               double sigTrial,
                               double rhoTrial,
                               double sig) const
{
...
double
CDPM2F2::computeDuctilityMeasure(double sig,
                                      double rho,
                                      double theta) const
{
...
double
CDPM2F2::computeRatioPotential(double sig,
                                    double rho,
...
double
CDPM2F2::performRegularReturn(FloatArrayF< 6 > &effectiveStress,
                                   double kappaP,
                                   GaussPoint *gp,
                                   double theta) const
{
...
FloatMatrixF< 3, 3 >
CDPM2F2::compute1dJacobian(double totalsigma,
                                double theta,
                                double kappa,
                                double deltaLambda,
                                GaussPoint *gp) const
...
FloatMatrixF< 4, 4 >
CDPM2F2::computeJacobian(double sig,
                              double rho,
                              double theta,
                              double kappa,
                              double deltaLambda,
...
double
CDPM2F2::computeYieldValue(double sig,
                                double rho,
                                double theta,
                                double tempKappa) const
{
...
double
CDPM2F2::computeDFDKappa(double sig,
                              double rho,
                              double theta,
                              double tempKappa,
                              bool mode1d) const
...
double
CDPM2F2::computeDFDInv1d(double sigma,
                              double theta,
                              double tempKappa) const
{
    double yieldHardOne = computeHardeningOne(tempKappa);
...
FloatArrayF< 2 >
CDPM2F2::computeDFDInv(double sig,
                            double rho,
                            double theta,
                            double tempKappa) const
{
...
double
CDPM2F2::computeDKappaDDeltaLambda1d(double sig, double theta, double tempKappa) const
{
...
double
CDPM2F2::computeDKappaDDeltaLambda(double sig,
                                        double rho,
                                        double theta,
                                        double tempKappa) const
{
...
double
CDPM2F2::computeDDKappaDDeltaLambdaDInv1d(double sigma, double theta, double tempKappa) const
{
...
FloatArrayF< 2 >
CDPM2F2::computeDDKappaDDeltaLambdaDInv(double sig,
                                             double rho,
                                             double theta,
                                             double tempKappa) const
{
...
double
CDPM2F2::computeDDKappaDDeltaLambdaDKappa(double sig, double rho, double theta, double tempKappa) const
{
    //Compute first and second derivative of plastic potential
    auto dGDInv = computeDGDInv(sig, rho, tempKappa);
...
double
CDPM2F2::computeDDKappaDDeltaLambdaDKappa1d(double sig,
                                                 double theta,
                                                 double tempKappa) const
{
...
double
CDPM2F2::computeDDuctilityMeasureDInv1d(double sigma,
                                             double theta,
                                             double tempKappa) const
{
    double thetaConst = pow(2. * cos(theta), 2.);
...
FloatArrayF< 2 >
CDPM2F2::computeDDuctilityMeasureDInv(double sig,
                                           double rho,
                                           double theta,
                                           double tempKappa) const
{
...
double
CDPM2F2::computeDGDInv1d(double sigma,
                              double tempKappa) const
{
    //compute yieldHard and yieldSoft
    double yieldHardOne = computeHardeningOne(tempKappa);
...
FloatArrayF< 2 >
CDPM2F2::computeDGDInv(double sig,
                            double rho,
                            double tempKappa) const
{
    //compute yieldHard and yieldSoft
...
double
CDPM2F2::computeDDGDInvDKappa1d(double sigma,
                                     double tempKappa) const
{
    //compute yieldHard and yieldSoft
    double yieldHardOne = computeHardeningOne(tempKappa);
...
FloatArrayF< 2 >
CDPM2F2::computeDDGDInvDKappa(double sig,
                                   double rho,
                                   double tempKappa) const
{
    //Compute dilation parameter
...
double
CDPM2F2::computeDDGDDInv1d(double sigma,
                                double tempKappa) const
{
...
FloatMatrixF< 2, 2 >
CDPM2F2::computeDDGDDInv(double sig,
                              double rho,
                              double tempKappa) const
{
    //compute yieldHardOne and yieldSoft
...
double
CDPM2F2::computeAlpha(FloatArrayF< 6 > &effectiveStressTension,
                           FloatArrayF< 6 > &effectiveStressCompression,
                           const FloatArrayF< 6 > &effectiveStress) const
{
...
double
CDPM2F2::computeHardeningOne(double kappa) const
{
    if ( kappa <= 0. ) {
...
double
CDPM2F2::computeHardeningOnePrime(double kappa) const
{
    if ( kappa <= 0. ) {
...
double
CDPM2F2::computeHardeningTwo(double kappa) const
{
    if ( kappa <= 0. ) {
        return 1.;
    } else if ( kappa > 0. && kappa < 1. ) {
...
double
CDPM2F2::computeHardeningTwoPrime(double kappa) const
{
    if ( kappa <= 0. ) {
        return 0.;
    } else if ( kappa >= 0. && kappa < 1. ) {
...
FloatMatrixF< 1, 1 >
CDPM2F2::give1dStressStiffMtrx(MatResponseMode mode, GaussPoint *gp, TimeStep *tStep) const
{
    auto status = static_cast< CDPM2F2Status * >( this->giveStatus(gp) );
...
FloatMatrixF< 6, 6 >
CDPM2F2::give3dMaterialStiffnessMatrix(MatResponseMode mode,
                                            GaussPoint *gp,
                                            TimeStep *tStep) const
{
...
FloatMatrixF< 6, 6 >
CDPM2F2::compute3dSecantStiffness(GaussPoint *gp, TimeStep *tStep) const
{
    auto status = static_cast< CDPM2F2Status * >( giveStatus(gp) );
...
void
CDPM2F2::computeCoordinates(const FloatArrayF< 6 > &stress, double &sigNew, double &rhoNew, double &thetaNew) const
{
...
void
CDPM2F2::assignStateFlag(GaussPoint *gp, TimeStep *tStep) const
{
    auto status = static_cast< CDPM2F2Status * >( this->giveStatus(gp) );
    ...
    if ( tempKappaP > kappaP ) {
        if ( tempKappaDTension > kappaDTension && (tempDamageTension > 0. || tempDamageCompression > 0.) ) {
            status->letTempStateFlagBe(CDPM2F2Status::CDPM2F2_PlasticDamage);
        } else {
            status->letTempStateFlagBe(CDPM2F2Status::CDPM2F2_Plastic);
        }
    } else {
        const int state_flag = status->giveStateFlag();
        if ( tempKappaDTension > kappaDTension && (tempDamageTension > 0. || tempDamageCompression > 0.) ) {
            status->letTempStateFlagBe(CDPM2F2Status::CDPM2F2_Damage);
        } else {
            if ( state_flag == CDPM2F2Status::CDPM2F2_Elastic ) {
                status->letTempStateFlagBe(CDPM2F2Status::CDPM2F2_Elastic);
            } else {
                status->letTempStateFlagBe(CDPM2F2Status::CDPM2F2_Unloading);
            }
        }
    }
}
...
FloatArrayF< 6 >
CDPM2F2::computeDRhoDStress(const FloatArrayF< 6 > &stress) const
{
...
FloatArrayF< 6 >
CDPM2F2::computeDSigDStress() const
{
    return {
        1. / 3., 1. / 3., 1. / 3., 0., 0., 0.
    };
...
FloatMatrixF< 3, 3 >
CDPM2F2::computeDDRhoDDStress(const FloatArrayF< 6 > &stress) const
{
    //compute volumetric deviatoric split
    auto deviatoricStress = computeDeviator(stress);
    double rho = computeSecondCoordinate(deviatoricStress);
...
int
CDPM2F2::giveIPValue(FloatArray &answer,
                          GaussPoint *gp,
...
MaterialStatus *
CDPM2F2::CreateStatus(GaussPoint *gp) const
{
    return new  CDPM2F2Status(gp);
}
} //end of namespace
